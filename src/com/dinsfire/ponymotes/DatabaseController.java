...
public class DatabaseController {
	...
	public void addEmote(String emoteName, String tags, int isNSFW, String dateModified, String source) {
	...
	public void addEmote(Emote emote) {
		...
		values.put(DatabaseOpenHelper.COLUMN_EMOTE_NAME, emote.getEmoteName());
		values.put(DatabaseOpenHelper.COLUMN_TAGS, emote.getTags());
		values.put(DatabaseOpenHelper.COLUMN_NSFW, emote.isNSFW());
		values.put(DatabaseOpenHelper.COLUMN_DATE_MODIFIED, emote.getDateModified());
		values.put(DatabaseOpenHelper.COLUMN_SOURCE, emote.getSource());
	...
	public void batchAddEmotes(ArrayList<Emote> toBeAdded) {
		...
		if(toBeAdded.size() > 0) {
			...
			String sql = "INSERT INTO " + DatabaseOpenHelper.TABLE_EMOTES + "  VALUES(?,?,?,?,?)";
	...
	public void batchDeleteEmotes(ArrayList<?> toBeDeleted) {
		...
		if(toBeDeleted.size() > 0) {
			emoteDatabase.beginTransaction();
			String sql = "DELETE FROM " + DatabaseOpenHelper.TABLE_EMOTES + " WHERE emoteName = (?)";
	        SQLiteStatement insert = emoteDatabase.compileStatement(sql);
	...
	public ArrayList<String> searchFor(String searchTerm, String limit) {
		...
		for(int i = 0; i < splited.length; i++) {
			if(splited[i].startsWith("+")){
				...
				if(splited[i].startsWith("-+")) {
					...
					query += DatabaseOpenHelper.COLUMN_EMOTE_NAME + " NOT LIKE ?";
					splited[i] = splited[i].replace("-","");
				}
		...
		Cursor cursor = emoteDatabase.query(DatabaseOpenHelper.TABLE_EMOTES, 
				new String[] {DatabaseOpenHelper.COLUMN_EMOTE_NAME}, 
				query, 
				splited, 
				null, null, 
				DatabaseOpenHelper.COLUMN_EMOTE_NAME, 
				limit);
		...
		while (cursor.isAfterLast() == false) {
			result.add(cursor.getString(cursor.getColumnIndex(DatabaseOpenHelper.COLUMN_EMOTE_NAME)));
			cursor.moveToNext();
		}
	...
	public Cursor getInfoForEmote(String emoteName) {
		...
		Cursor cursor = emoteDatabase.query(
				DatabaseOpenHelper.TABLE_EMOTES, 
				columns, 
				where, 
				arguments, 
				null, 
	...
	public int getNumberOfRows() {
		...
		Cursor cursor = emoteDatabase.rawQuery("SELECT * FROM " + DatabaseOpenHelper.TABLE_EMOTES, null);
	...
	public ArrayList<String> getDifferences(boolean downloadEmotes, boolean downloadMissingEmotes, boolean includeNSFW, DatabaseController localDB) {
		...
		{
			String emoteName = newCursor.getString(newCursor.getColumnIndex(DatabaseOpenHelper.COLUMN_EMOTE_NAME));
			int dateModified = newCursor.getInt(newCursor.getColumnIndex(DatabaseOpenHelper.COLUMN_DATE_MODIFIED));
			String tags = newCursor.getString(newCursor.getColumnIndex(DatabaseOpenHelper.COLUMN_TAGS));
			int isNSFW = newCursor.getInt(newCursor.getColumnIndex(DatabaseOpenHelper.COLUMN_NSFW));
			String source = newCursor.getString(newCursor.getColumnIndex(DatabaseOpenHelper.COLUMN_SOURCE));
			
			Emote newEmote = new Emote(emoteName, tags, isNSFW, dateModified, source);
			...
			if(!newEmote.isNSFW() || includeNSFW ) {
				...
				if(oldCursor.getCount() > 0) {
					//String oldEmoteName = oldCursor.getString(oldCursor.getColumnIndex(DatabaseOpenHelper.COLUMN_EMOTE_NAME));
					int oldDateModified = oldCursor.getInt(oldCursor.getColumnIndex(DatabaseOpenHelper.COLUMN_DATE_MODIFIED));
					//System.out.println("Checking to see if " + oldEmoteName + " needs updating by comparing " + oldDateModified + " to " + dateModified);
					...
					if(dateModified > oldDateModified) {
						...
						if(downloadMissingEmotes) {
							if(!FileIO.emoteExistsInStorage(emoteName)) {
								//Log.i("database", "Downloading missing: " + emoteName);
								toDownload.add(newEmote);
							}
						}
					}
				} else {
					//Since the emote isn't in the table, add to the database and download
					
					//System.out.println("Not in local database: " + emoteName);
		...
		ArrayList<String> stringNewEmotes = new ArrayList<String>(newEmotes.size());
		ArrayList<String> stringToDownload = new ArrayList<String>(toDownload.size());
		...
		//TODO: update this code. Implement the check somewhere else. SOMETHING.
		if(downloadEmotes) {
			if(downloadMissingEmotes)
				stringNewEmotes.addAll(stringToDownload);
			
			Log.i("database", "Need to download: " + newEmotes.size() + " out of " + String.valueOf(getNumberOfRows()));
